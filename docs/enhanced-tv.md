...
- Install the ARM32 preview alongside the main app. Launch it and confirm that
  the preview has its own profile and source settings.
...
TMDB, Trakt, SIMKL and account features that need build-time credentials require
a separately configured build for end-to-end testing. Do not put provider
passwords, tokens or private playlist URLs in bug reports.
